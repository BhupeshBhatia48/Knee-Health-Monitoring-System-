# train_cnn_mlp_knee_health.py
# -------------------------------------------------------
# 1D CNN + MLP for knee health classification with timing analysis
...
class TimedDense(layers.Layer):
    def __init__(self, units, **kwargs):
        super(TimedDense, self).__init__()
        self.dense = layers.Dense(units, **kwargs)
        self.timing_data = []
        
    def call(self, inputs, training=None):
        start_time = time.perf_counter()
        result = self.dense(inputs, training=training)
        end_time = time.perf_counter()
        if training:  # Only track during training
            self.timing_data.append(end_time - start_time)
        return result
...
# -----------------------
# Build Model (CNN + MLP + Static) with Timing
# -----------------------
def build_cnn_mlp_model(seq_len: int, n_channels: int, static_dim: int, n_classes: int = 3) -> Tuple[Model, List]:
    ...
    # MLP layers with timing (replacing LSTM layers)
    mlp1 = TimedDense(256, activation='relu', name='mlp_1')
    timed_layers.append(('MLP_1', mlp1))
    x = mlp1(x)
    x = layers.Dropout(0.25)(x)
    
    mlp2 = TimedDense(128, activation='relu', name='mlp_2')
    timed_layers.append(('MLP_2', mlp2))
    x = mlp2(x)
...
def analyze_timing_results(timed_layers):
    """
    Analyze and display timing results for CNN and MLP layers.
    """
    print("\n" + "="*60)
    ...
    for layer_name, layer in timed_layers:
        if hasattr(layer, 'timing_data') and layer.timing_data:
            ...
            if 'CNN' in layer_name:
                ...
                mlp_times.extend(layer.timing_data)
    
    # Overall timing comparison
    total_cnn_time = sum(cnn_times) if cnn_times else 0
    total_mlp_time = sum(mlp_times) if mlp_times else 0
    total_time = total_cnn_time + total_mlp_time
    
    print("\n" + "="*40)
    print("OVERALL TIMING COMPARISON")
    print("="*40)
    print(f"Total CNN Time:     {total_cnn_time:.4f} seconds ({len(cnn_times)} forward passes)")
    print(f"Total MLP Time:     {total_mlp_time:.4f} seconds ({len(mlp_times)} forward passes)")
    print(f"Combined Time:      {total_time:.4f} seconds")
    
    if total_time > 0:
        cnn_percentage = (total_cnn_time / total_time) * 100
        mlp_percentage = (total_mlp_time / total_time) * 100
        print(f"\nTime Distribution:")
        print(f"CNN Layers:         {cnn_percentage:.1f}% of total time")
        print(f"MLP Layers:         {mlp_percentage:.1f}% of total time")
        
        # Speed comparison
        if total_mlp_time > 0:
            speed_ratio = total_cnn_time / total_mlp_time
            if speed_ratio > 1:
                print(f"\nSpeed Analysis:     CNNs are {speed_ratio:.2f}x slower than MLPs")
            else:
                print(f"Speed Analysis:     MLPs are {1/speed_ratio:.2f}x slower than CNNs")
...
def main():
    ...
    model, timed_layers = build_cnn_mlp_model(seq_len=SEQ_LEN, n_channels=X_ts.shape[2], static_dim=X_static.shape[1], n_classes=3)
    model.summary()
    ...
    cb = [
        ...
        callbacks.ModelCheckpoint("best_cnn_mlp.keras", monitor='val_loss', save_best_only=True)
    ]
